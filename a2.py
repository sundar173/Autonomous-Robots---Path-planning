...
class AStarSearch:
    ...
    def get_possible_moves(self):
        ...
        for move in potential_moves:
            ...
            if str(move) not in self.explored and str(move) not in self.not_explored:
                ...
                # self.path[move[0],move[1]] = self.pos_depth+1+ self.heuristic(move)
    ...
    def goal_found(self):
        if self.goal_str in self.not_explored:
            ...
            self.path[self.pos[0],self.pos[1]] = self.pos_depth - self.heuristic(self.pos)
            self.h_grid[self.pos[0], self.pos[1]] = self.pos_depth + 1 + self.heuristic(self.pos)
    ...
    def heuristic(self, move):
        # x = abs(goal[0]-move[0])
        # y = abs(goal[1]-move[1])
        # answer = np.sqrt(x**2+y**2)
        distance  = goal - move
        answer = np.sqrt(sum(distance*distance))
